...
def test_e2e_first_import_creates_tree(tmp_path: Path, fixture_zip: Path):
    ...
    assert rc == 0
    files = sorted(p.relative_to(root).as_posix() for p in root.rglob("*.md"))
...
def test_e2e_second_import_is_idempotent(tmp_path: Path, fixture_zip: Path):
    ...
    snapshot1 = {p.relative_to(root).as_posix(): p.read_text() for p in root.rglob("*.md")}

    rc = cli_main(["import", "flomo", str(fixture_zip), "--root", str(root)])
    assert rc == 0
    snapshot2 = {p.relative_to(root).as_posix(): p.read_text() for p in root.rglob("*.md")}
...
def test_e2e_full_mirror_handles_deletion(tmp_path: Path, simple_memos_html: bytes):
    ...
    files_before = sorted(p.relative_to(root).as_posix() for p in root.rglob("*.md"))
    ...
    assert rc == 0
    files_after = sorted(p.relative_to(root).as_posix() for p in root.rglob("*.md"))
